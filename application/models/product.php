<?php class Product extends CI_Model {
	...
	public function create($post,$data){
		...
		public function update_products_new_cat_update(){
			...
			$updateCategoryData=array('product_id'=>$post['produc_id'],'categories_id'=>$post['category'],'updated_at'=>now());

			$this->db->where('categories_have_products.product_id',$post['producid']);
